...
*/
Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, uint16_t p, neoPixelType t) : begun(false), brightness(0), pixels(NULL), opixels(NULL), brightfr(NULL), brightfg(NULL), brightfb(NULL), brightfw(NULL)
{
...
Adafruit_NeoPixel::~Adafruit_NeoPixel()
{
  ...
  if (pio_no_sm[pio_get_index(pio)] == 0)
  { // if no sm on the pio instance, remove the program
...
void Adafruit_NeoPixel::rp2040Init(uint8_t set_pin)
{
  ...
  if (resultsm != -1)
  {
    if (pio0_offset == -1)
    {
      ...
      PRINTF1("canpio = %d\n", canpio);
      if (canpio)
        pio0_offset = pio_add_program(pio0, &ws2812byte_program);
    };
    pio = pio0;
  };
  if (resultsm == -1 || pio0_offset == -1)
  {
    ...
    if (resultsm != -1)
    {
      if (pio1_offset == -1)
      {
        ...
        if (canpio)
          pio1_offset = pio_add_program(pio0, &ws2812byte_program);
      };
      pio = pio1;
    }
  };
...
uint32_t Adafruit_NeoPixel::getPixelColor(uint16_t n) const
{
  ...
  { // else take the values from the original array
    if (wOffset == rOffset)
    { // Is RGB-type device
      po = &opixels[n * 3];
